...
const HeroSection = () => {
  ...
  return (
    ...
    >
      ...
      <div
        className="absolute top-[40%] left-[20%] w-32 h-32 bg-indigo-300 rounded-full opacity-20 blur-3xl animate-float -z-10"
        style={{ animationDelay: '2s' }}
      ></div>

      <div className="max-w-7xl mx-auto px-6 grid md:grid-cols-2 gap-12 w-full">
        <div className="flex flex-col justify-center">
          <span className="text-sm md:text-base font-medium px-4 py-1.5 bg-violet-100 text-violet-800 rounded-full inline-block w-fit mb-6 fade-in">
            Bienvenida a una nueva forma de conexión
          </span>
          <h1
            className="text-4xl md:text-5xl lg:text-6xl font-display font-bold text-gray-900 leading-tight mb-6 fade-in"
            style={{ transitionDelay: '100ms' }}
          >
            Conectando <span className="text-gradient">Mujeres</span>, Empoderando{' '}
            <span className="text-gradient">Vidas</span>
          </h1>
          <p
            className="text-lg text-gray-700 mb-8 max-w-lg fade-in"
            style={{ transitionDelay: '200ms' }}
          >
            Únete a nuestra vibrante comunidad de mujeres que se apoyan mutuamente,
            comparten experiencias y crecen juntas en un espacio seguro y empoderador.
          </p>

          {/* Botones */}
          <div className="flex flex-wrap gap-4 fade-in" style={{ transitionDelay: '300ms' }}>
            {/* Botón "Únete Ahora" -> /under-construction */}
            <Link to="/under-construction">
              <Button
                size="lg"
                className="rounded-full bg-gradient-violet hover:opacity-90 shadow-lg btn-hover text-base px-8 h-12"
              >
                Únete Ahora
              </Button>
            </Link>

            {/* Botón "Saber Más" -> /under-construction */}
            <Link to="/under-construction">
              <Button
                variant="outline"
                size="lg"
          ...
          <div className="mt-10 fade-in" style={{ transitionDelay: '400ms' }}>
            <p className="text-sm text-gray-500 mb-2">
              De confianza para mujeres de todo el mundo
            </p>
            <div className="flex items-center gap-4">
              <div className="flex -space-x-3">
                <div className="w-8 h-8 rounded-full bg-violet-300 border-2 border-white flex items-center justify-center text-white text-xs font-medium">
                  JD
                </div>
                <div className="w-8 h-8 rounded-full bg-pink-300 border-2 border-white flex items-center justify-center text-white text-xs font-medium">
                  SR
                </div>
                <div className="w-8 h-8 rounded-full bg-indigo-400 border-2 border-white flex items-center justify-center text-white text-xs font-medium">
                  MP
                </div>
                <div className="w-8 h-8 rounded-full bg-violet-500 border-2 border-white flex items-center justify-center text-white text-xs font-medium">
                  +
                </div>
              </div>
              <span className="text-sm text-gray-600">
                Únete a más de 10,000 miembros
              </span>
            </div>
          </div>
        </div>
